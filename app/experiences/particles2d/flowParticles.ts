 ...
 * Adapted from three-fluid-fx examples/extras/particles/glsl/flowParticles.ts.
 *
 * Pipeline per frame:
...
export interface FlowParticlesOptions {
  mode: ParticleMode;
  size?: number;
  customDestinations?: Float32Array;
}
...
export interface FlowParticles {
  ...
  setPalette: (mode: number, params?: { bias: [number, number, number]; amp: [number, number, number]; phase: [number, number, number] }) => void;
  dispose: () => void;
}
...
void main() {
  ...
  if (uPaletteMode == 0) {
    float phase = a * 6.3;
    palette = vec3(cos(phase), cos(phase + 83.0), cos(phase + 21.0)) * 0.56 + 0.55;
    ...
    float phase = a * 6.28318530718;
    palette = uPaletteBias + uPaletteAmp * cos(phase + uPalettePhase);
  }
...
export function createFlowParticles(
  ...
  if (options.customDestinations) {
    data.destinations = options.customDestinations;
    data.positions = new Float32Array(options.customDestinations);
    for (let i = 3; i < data.positions.length; i += 4) {
      data.positions[i] = 1;
    }
  }
  ...
  const copyPass = new FullscreenPass(copyMaterial as any);
  const velocityPass = new FullscreenPass(velocityMaterial as any);
  const positionPass = new FullscreenPass(positionMaterial as any);

  copyMaterial.uniforms.uSource.value = initialPositionTexture;
  copyPass.render(renderer as any, position.read as any);
  copyPass.render(renderer as any, position.write as any);
  copyMaterial.uniforms.uSource.value = initialVelocityTexture;
  copyPass.render(renderer as any, velocity.read as any);
  copyPass.render(renderer as any, velocity.write as any);
  ...
  const renderMaterial = new ShaderMaterial({
    ...
    uniforms: {
      ...
      uPaletteMode: new Uniform(0),
      uPaletteBias: new Uniform(new Vector3(0.55, 0.55, 0.55)),
      uPaletteAmp: new Uniform(new Vector3(0.56, 0.56, 0.56)),
      uPalettePhase: new Uniform(new Vector3(0.0, 83.0, 21.0)),
    },
    transparent: true,
    depthTest: true,
    depthWrite: true,
  ...
  return {
    points,
    setPalette(mode: number, params?: { bias: [number, number, number]; amp: [number, number, number]; phase: [number, number, number] }) {
      renderMaterial.uniforms.uPaletteMode.value = mode;
      if (params) {
        (renderMaterial.uniforms.uPaletteBias.value as Vector3).set(params.bias[0], params.bias[1], params.bias[2]);
        (renderMaterial.uniforms.uPaletteAmp.value as Vector3).set(params.amp[0], params.amp[1], params.amp[2]);
        (renderMaterial.uniforms.uPalettePhase.value as Vector3).set(params.phase[0], params.phase[1], params.phase[2]);
      }
    },
    step(params) {
      ...
      velocityPass.render(renderer as any, velocity.write as any);
      swap(velocity);
      ...
      positionPass.render(renderer as any, position.write as any);
      swap(position);
      renderer.setRenderTarget(null);
